...
-----------------------------------------------------------------------------------------------#>
$docPath = "./Help"
New-MarkdownHelp -Module SecuredArcana `
...
# Now loop over each one fixing the issue
  foreach ($f in $FilePath)
  {
    ...
    foreach ($line in $contents)
    {
      ...
      if ($line -eq '{{ Fill ProgressAction Description }}')
      {
        $line = 'A common parameter. The action to take when the progress is updated.'
      }
      ...
      $lookFor = '[https://github.com/arcanecode/ArcaneBooks/blob/master/Documentation/'
      ...
      $replacements = @{'Author: Robert C Cain | @ArcaneCode | arcane@arcanetc.com' = 'Author: Robert C Cain | [@ArcaneCode](https://twitter.com/arcanecode) | arcane@arcanetc.com'
                        '[https://github.com/arcanecode/SecuredArcana](https://github.com/arcanecode/SecuredArcana)' = '[SecuredArcana on GitHub](https://github.com/arcanecode/SecuredArcana)'
    ...
    {
      if ($line -eq '[ArcaneCode''s Website](http://arcanecode.me)')
      {
